...
from keras.layers import Conv2D, MaxPooling2D, Conv2DTranspose, Reshape, Dense
...
def makeModel(j):
	model = load_model(save_file, custom_objects={'DSSIM': DSSIM})
	#model = load_model('r16.3.ResetLastTwoLayers.h5', custom_objects={'DSSIM': DSSIM})

	Model = Sequential()
	for i in range(len(model.layers)):
		model.layers[i].trainable = True
		model.layers[i].data_format = 'channels_last'
		if i == 6:
			Model.add(Conv2DTranspose(512,strides = (1,1), kernel_size = (5,5), activation = 'relu', name = 'NEWDECONV'))
		if i == 5:
			Model.add(Conv2D(1024,strides = (1,1), kernel_size = (5,5), activation = 'sigmoid', name = 'NEWCONV'))
		if i != 6 and i != 5:
			model.layers[i].trainable = False
			Model.add(model.layers[i])
...
def loadData(size = 1000, size_ = batch_size):
	...
	ran = np.random.randint(0,len(os.listdir(data_direct))/11, size=size_)
	for i in range(len(ran)):
		x = np.load(data_direct+'data'+str(ran[i])+'.1.npy')
		x = np.swapaxes(x,0,1)
		x = np.swapaxes(x,1,2)
		X[i] = x

		y = data_direct+'data'+str(ran[i])+'.2.npy'
                y = np.load(y)
                y = np.swapaxes(y,0,1)
		y = np.swapaxes(y,1,2)
		Y[i] = y
...
if __name__ == "__main__":
	print("start")
	DSSIM1 = DSSIMObjective(kernel_size=16)
	DSSIM2 = DSSIMObjective(kernel_size=64)

	def DSSIM(y_true,y_pred):
		#return K.log(dsimw*DSSIM1(y_true,y_pred) + 25./3*maew*K.mean(K.abs(y_true-y_pred))) + dsimw*DSSIM1(y_true,y_pred) + 25./3*maew*K.mean(K.abs(y_true-y_pred))
		#return K.mean(((y_true-y_pred)/0.01)**10) + K.mean(K.abs(y_true-y_pred))
		#return  K.mean(K.abs(y_true-y_pred)) + 1/K.mean(K.abs(y_pred))
		d1 = DSSIM1(y_true, y_pred)
		d2 = DSSIM2(y_true, y_pred)
		return d1 + d2 + maew*K.mean(K.abs(y_true-y_pred))
	...
	if not predict_:
		...
		if load_:
			print('Model Loading')
			model = load_model(load_file, custom_objects={'DSSIM': DSSIM})
			for i in range(len(model.layers)):
                		model.layers[i].trainable = True
		...
		for i in range(trainiter):
			...
			print('Data Loaded')
			model.fit(X,Y,epochs=epochs_,verbose=2, batch_size=batch_size)
			x = X[0].reshape((1,256,256,4))
			y = model.predict(x)
			np.save('Predict',y)
			np.save('Label',Y[0].reshape((1,256,256,4)))
        		np.save('Input',x)
	...
	if predict_:
		...
		print('Model Loaded')
		dp = np.random.randint(0,4000)
		X_ = np.load(data_direct+'data'+str(dp)+".1.npy")
		X_ = np.swapaxes(X_,0,1)
		X_ = np.swapaxes(X_,1,2)
		X_ = X_.reshape((1,256,256,4))
		Y_ = np.load(data_direct+'data'+str(dp)+".2.npy")
		Y_ = np.swapaxes(Y_,0,1)
		Y_ = np.swapaxes(Y_,1,2)
		Y_ = Y_.reshape((1,256,256,4))
		P_ = model.predict(X_)
		np.save('Label',Y_)
		np.save('Predict',P_)
		np.save('Input',X_)
